  ...
  const counterObserver = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
      ...
      if (counter && !counter.dataset.animated) {
        animateCounter(counter, 1800); // duration in ms
...
/**
 * Animate numeric portion of an element's text content.
 * Supports "10+", "1,200", "50%", "12.5K" (it extracts the numeric part only).
 */
function animateCounter(el, duration = 2000) {
  const text = el.textContent.trim();

  // find first numeric substring (supports decimals and commas)
  const match = text.match(/-?\d[\d,]*\.?\d*/);
  if (!match) return; // nothing numeric found

  const numStr = match[0].replace(/,/g, ''); // remove commas
  const endVal = parseFloat(numStr);
  if (isNaN(endVal)) return;

  // preserve prefix and suffix (anything before/after the numeric substring)
  ...
  function step(now) {
    ...
    // optional easing (easeOutQuad)
    const eased = 1 - Math.pow(1 - progress, 2);

    let current = eased * (endVal - startVal) + startVal;

    // Format value: integers rounded, decimals preserved (1 decimal)
    if (isInteger) {
      current = Math.round(current);
      el.textContent = prefix + current.toLocaleString() + suffix;
    } else {
      // keep 1 or 2 decimal places depending on original
      const decimals = (numStr.indexOf('.') > -1) ? (numStr.split('.')[1].length) : 1;
    ...
    if (progress < 1) {
      ...
      el.textContent = prefix + (isInteger ? endVal.toLocaleString() : endVal.toLocaleString()) + suffix;
    }
  }
