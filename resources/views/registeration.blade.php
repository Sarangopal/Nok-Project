...
hiddenInput.removeAttribute('required'); // ensure it's never required
...
memberSwitch.addEventListener('change', function(){
    if (this.checked) {
        ...
        // Make only existing-member fields required
        existingFields.querySelectorAll('input').forEach(input => {
            input.setAttribute('required', 'required');
        });

        // remove required from member_type (if accidentally added)
        ...
        // remove required from member_type (just to be sure)
        hiddenInput.removeAttribute('required');
    }
});
